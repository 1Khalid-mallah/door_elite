...
export const AppProvider = ({ children }) => {
  ...
  const fetchServices = useCallback(async (params = {}) => {
    try {
      ...
      const url = queryString ? `/service/list?${queryString}` : '/service/list';
  ...
  const fetchUserBookings = useCallback(async () => {
    try {
      ...
      const { data } = await axiosInstance.get('/booking/user');
  ...
  const addBooking = useCallback(async (bookingData) => {
    try {
      ...
      if (data.success) {
        ...
        try {
          const bookingData = await axiosInstance.get('/booking/user');
  ...
  const updateBooking = useCallback(async (bookingId, bookingData) => {
    try {
      setBookingLoading(true);
      const { data } = await axiosInstance.put(`/booking/${bookingId}`, bookingData);
  ...
  const cancelBooking = useCallback(async (bookingId) => {
    try {
      setBookingLoading(true);
      const { data } = await axiosInstance.put(`/booking/${bookingId}/cancel`);
  ...
  const completeBooking = useCallback(async (bookingId, rating, review) => {
    try {
      setBookingLoading(true);
      const { data } = await axiosInstance.put(`/booking/${bookingId}/complete`, { rating, review });
  ...
  const updateProfile = useCallback(async (formData) => {
    try {
      ...
      const { data } = await axiosInstance.put('/user/me', formData, {
        headers: {
